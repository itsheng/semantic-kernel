...
def test_default_groq_chat_prompt_execution_settings():
    settings = GroqChatPromptExecutionSettings()
...
def test_custom_groq_chat_prompt_execution_settings():
    settings = GroqChatPromptExecutionSettings(
        temperature=0.5,
        top_p=0.5,
        max_tokens=128,
...
def test_groq_chat_prompt_execution_settings_from_default_completion_config():
    settings = PromptExecutionSettings(service_id="test_service")
    chat_settings = GroqChatPromptExecutionSettings.from_prompt_execution_settings(settings)
...
def test_groq_chat_prompt_execution_settings_from_openai_prompt_execution_settings():
    chat_settings = GroqChatPromptExecutionSettings(service_id="test_service", temperature=1.0)
    new_settings = GroqChatPromptExecutionSettings(service_id="test_2", temperature=0.0)
...
def test_groq_chat_prompt_execution_settings_from_custom_completion_config():
...
def test_create_options_with_function_choice_behavior():
    with pytest.raises(NotImplementedError):
        GroqChatPromptExecutionSettings(
            service_id="test_service",
